...
//import org.onosproject.yang.gen.v1.openconfigspanningtree.rev20170714.openconfigspanningtree.stprapidpvsttop.Vlan;
...
public class TerminalDeviceFlowRule extends DefaultFlowRule {
    ...
    //As generated by the TSON client flow rules
    private static final int NUM_CRITERIA_CLIENT_TSON_RULES = 2;
    private static final int NUM_INSTRUCTIONS_CLIENT_TSON_RULES = 1;

    //As generated by the TSON line flow rules
    private static final int NUM_CRITERIA_LINE_TSON_INGRESS_RULE = 2;
    private static final int NUM_INSTRUCTIONS_LINE_TSON_INGRESS_RULE = 2;
    ...
    private HashMap<PortNumber, Pair<VlanId, VlanId>> flowVlanPortAssignment;
    private VlanId vlanIdIn;
    private VlanId vlanIdOut;
    ...
    public TerminalDeviceFlowRule(FlowRule rule, List<PortNumber> linePorts) {
        ...
        checkArgument((criteria.size() == NUM_CRITERIA_LINE_EGRESS_RULE) ||
                ...
                (criteria.size() == NUM_CRITERIA_CLIENT_TSON_RULES) ||
                (criteria.size() == NUM_CRITERIA_LINE_TSON_INGRESS_RULE),
        ...
        checkArgument((instructions.size() == NUM_INSTRUCTIONS_LINE_EGRESS_RULE) ||
                        ...
                        (instructions.size() == NUM_INSTRUCTIONS_CLIENT_TSON_RULES) ||
                        (instructions.size() == NUM_INSTRUCTIONS_LINE_TSON_INGRESS_RULE),
        ...
        if ((criteria.size() == NUM_CRITERIA_LINE_EGRESS_RULE) &&
            ...
            log.debug("Building the TerminalDeviceFlowRule LINE_EGRESS");
        ...
        //This is INGRESS or EGRESS rule on the CLIENT TSON side
        if ((criteria.size() == NUM_CRITERIA_CLIENT_TSON_RULES) &&
                (instructions.size() == NUM_INSTRUCTIONS_CLIENT_TSON_RULES)){
            ...
            try{
                flowVlanPortAssignment = criteria.stream()
                        .filter(c -> c instanceof ExtensionCriterion)
                        .map(c -> ((ExtensionCriterion) c).extensionSelector())
                        .findAny()
                        .get()
                        .getPropertyValue("vlanPortAssignment");
                vlanIdIn = flowVlanPortAssignment.get(inPortNumber).getKey();
                vlanIdOut = flowVlanPortAssignment.get(inPortNumber).getValue();
            }
            catch(ExtensionPropertyException exception){
                log.debug("ExtensionPropertyException caught: " + exception);
            }
            ...
            if (linePorts.contains(outPortNumber)) {
                type = Type.CLIENT_TSON_INGRESS;
            } else {
                type = Type.CLIENT_TSON_EGRESS;
            }

        }

        if ((criteria.size() == NUM_CRITERIA_LINE_TSON_INGRESS_RULE) &&
                (instructions.size() == NUM_INSTRUCTIONS_LINE_TSON_INGRESS_RULE)){

            log.debug("Building the TerminalDeviceFlowRule LINE_TSON_INGRESS");
            type = Type.LINE_TSON_INGRESS;

            criteria.forEach(
                    c -> checkArgument(
                            c instanceof PortCriterion || c instanceof ExtensionCriterion,
                            "Incompatible flow rule criteria for ADD TerminalDevice: " + criteria
                    )
            );
        ...
        if (type == Type.CLIENT_TSON_EGRESS) {
            connectionName = "ClientTSONEgress-LinePort-" + inPortNumber.toString()
                    + "-ClientPort-" + outPortNumber.toString();
        }
        if (type == Type.CLIENT_TSON_INGRESS) {
            connectionName = "ClientTSONIngress-ClientPort-" + inPortNumber.toString()
                    + "-LinePort-" + outPortNumber.toString();
        }
    ...
    public VlanId clientVlanIn() {
        return vlanIdIn;
    }

    public VlanId clientVlanOut() {
        return vlanIdOut;
    }

    public HashMap<PortNumber, Pair<VlanId, VlanId>> getFlowVlanPortAssignment() {
        return flowVlanPortAssignment;
    }
