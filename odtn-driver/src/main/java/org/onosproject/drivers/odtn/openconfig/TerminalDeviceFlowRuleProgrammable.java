...
public class TerminalDeviceFlowRuleProgrammable
    ...
    private void setLogicalChannel(NetconfSession session, String operation, String logChannel,
                                   HashMap<PortNumber, Pair<VlanId, VlanId>> flowVlanPortAssignment)
            throws NetconfException {
    ...
    private void setLogicalChannelAssignmentTSON(NetconfSession session, String operation, String client, String line,
                                             String assignmentIndex, String allocationIndex, String vlanIn,
                                                 String vlanOut)
            throws NetconfException {
        StringBuilder sb = new StringBuilder();
        ...
        sb.append("</assignment>");
        sb.append("</logical-channel-assignments>");
        sb.append("<vlanin>" + vlanIn + "</vlanin>");
        sb.append("<vlanout>" + vlanOut + "</vlanout>");
        sb.append("</channel>");
        sb.append("</logical-channels>");
        sb.append("</terminal-device>");
    ...
    protected boolean applyFlowRule(NetconfSession session, TerminalDeviceFlowRule rule) {
        ...
        if (rule.type == TerminalDeviceFlowRule.Type.LINE_INGRESS ||
                rule.type == TerminalDeviceFlowRule.Type.LINE_EGRESS ||
                    rule.type == TerminalDeviceFlowRule.Type.LINE_TSON_INGRESS) {
            ...
            try {
                if(rule.type != TerminalDeviceFlowRule.Type.LINE_TSON_INGRESS){
                    setLogicalChannel(session, OPERATION_ENABLE, componentName);
                }
                else{
                    setLogicalChannel(session, OPERATION_ENABLE, componentName, rule.getFlowVlanPortAssignment());
                }
            } catch (NetconfException e) {
        ...
        if (rule.type == TerminalDeviceFlowRule.Type.CLIENT_TSON_INGRESS ||
                rule.type == TerminalDeviceFlowRule.Type.CLIENT_TSON_EGRESS) {
            ...
            log.info("Sending TSON CLIENT FlowRule to device {} CLIENT port: {}, LINE port {}",
                    did(), clientPortName, linePortName);

            try {
                setLogicalChannelAssignmentTSON(session, OPERATION_ENABLE, clientPortName, linePortName,
                        DEFAULT_ASSIGNMENT_INDEX, DEFAULT_ALLOCATION_INDEX, rule.clientVlanIn().toString(),
                        rule.clientVlanOut().toString());
            } catch (NetconfException e) {
                log.error("Error setting the logical channel assignment");
                return false;
            }
